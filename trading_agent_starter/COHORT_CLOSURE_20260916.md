...
The gate verifies the manifest's file hash, canonical scope and agreement with the packet. Changing any target, date, SPY context, selection or clock rule invalidates scoped reviews. Refreezing the same file fails. A later scope change requires a new explicit documented amendment referencing this frozen scope and new scoped reviews; editing review hashes alone cannot authorize a changed scope. Quote-policy reviews also bind to the validator code hash. Targeted Git attributes preserve the frozen JSON bytes and gate/quote-validator LF bytes on Windows checkouts; no trading foundation logic is changed.
